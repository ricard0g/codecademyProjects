...
function validateCred(arr) {
	//create a reversed copy of our passed array because we don't want to mutate our original array and also want to work in a normal order from left to right and not in reverse
	let ourArr = arr.reverse();

    //for loop to iterate over our reversed array. By working on the reversed array I am accomplishing the direction of the iteration asked by the algorithm
	for (let i = 0; i < ourArr.length; i++) {
        //My logic here is that the index directly indicates the position of our number (in an array => 0-indexed). So, because we have to double only every other digit - taking into account that I'll work with the 0-based index - only odd numbers will be doubled for the algorithm to work.
        //So that's the condition that has to be met in the control flow. Only those index numbers that divided by 2 have a remainder distinct from 0 are going to be doubled.
		if (i % 2 !== 0) {
			ourArr[i] *= 2;
			if (ourArr[i] > 9) {
				ourArr[i] -= 9;
...
const idInvalidCardCompanies = (invalidNumsArr) => {
	const companiesArr = [
		...
		"Mastercard",
		"Discover",
	]; //save all the companies names in an array to avoid creating multiple variables that are all related
	...
	for (let i = 0; i < invalidNumsArr.length; i++) {
		...
		if (
			invalidNumsArr[i][0] === 3 &&
			invalidCompaniesArr.indexOf(companiesArr[0]) < 0
		) {
			invalidCompaniesArr.push(companiesArr[0]);
		} else if (
			invalidNumsArr[i][0] === 4 &&
			invalidCompaniesArr.indexOf(companiesArr[1]) < 0
		) {
			invalidCompaniesArr.push(companiesArr[1]);
		} else if (
			invalidNumsArr[i][0] === 5 &&
			invalidCompaniesArr.indexOf(companiesArr[2]) < 0
		) {
			invalidCompaniesArr.push(companiesArr[2]);
		} else if (
			invalidNumsArr[i][0] === 6 &&
			invalidCompaniesArr.indexOf(companiesArr[3]) < 0
		) {
